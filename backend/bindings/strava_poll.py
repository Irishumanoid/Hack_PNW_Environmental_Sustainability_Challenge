...
def get_athlete():
    if request.method == 'GET':
        ...
        athlete["score"] = recommend.get_user_score(recommend.vals_to_list(open("bindings/activity_data.json").read()))
...
def get_relavant_segments():
    if request.method == 'GET':
        ...
        return "💀" + json.dumps(userify_data(relavant_segments))

#I can't be bothered to pass this in somehow, set it before calling routes.sort
user_score = 0
def sortRouteFunction(route):
    return math.sqrt(math.pow(user_score, 2)+math.pow(route["difficulty"], 2))
...
def get_relavant_segments_inner(bounding_width, bounding_center, token):
    ...
    routes = make_api_call("segments/explore?bounds="+bounds_string, token).get("segments")
    for segment in routes:
        segment["difficulty"] = recommend.get_single_route_difficulty(segment)
    user_score = recommend.get_user_score(recommend.vals_to_list(open("bindings/activity_data.json").read()))
    routes.sort(key=sortRouteFunction, reverse=True)
    return routes
...
def userify_data(segments):
    ...
    for segment in segments:
        ...
        new_segment["difficulty"] = segment.get("difficulty")
